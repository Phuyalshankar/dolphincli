...
// ============================================================
// 🐬 DOLPHIN-ROUTING: File-based Auto Routing System
// ============================================================

/**
 * Recursively scan a pages directory and return route descriptors.
 * Conventions:
 *   index.jsx        → parent path (/ or /blog)
 *   [slug].jsx       → dynamic :slug
 *   _layout.jsx      → skipped (used as wrapper, not a route)
 *   folder/index.jsx → /folder
 */
function generateRoutesFromFiles(dir, basePath = '', projectRoot = '') {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      const subRoutes = generateRoutesFromFiles(
        fullPath,
      ...
      const nameWithoutExt = entry.name.replace(/\.(jsx|tsx|js|ts)$/, '');
      let routePath = path
        .join(basePath, nameWithoutExt)
        .replace(/\\/g, '/');
      ...
      // Relative import path from project root src/
...
/**
 * Generate the content of src/generated-router.jsx
 */
function generateRouterCode(routes, routingConfig = {}) {
  const {
    lazyLoad = true,
    basePath = '/',
    notFoundComponent = ''
  ...
  const imports = routes
    .map((r, i) =>
      lazyLoad
        ? `const Route${i} = lazy(() => import('${r.importPath}'));`
        : `import Route${i} from '${r.importPath}';`
    )
    .join('\n');

  const routeConfigs = routes
    .map((r, i) => `  { path: '${r.path}', element: <Route${i} /> }`)
    .join(',\n');

  const notFoundImport = notFoundComponent
    ? `import NotFound from '${notFoundComponent}';`
    : '';
  const notFoundRoute = notFoundComponent
    ? `,\n  { path: '*', element: <NotFound /> }`
    : '';

  const lazyImport = lazyLoad ? "import { lazy, Suspense } from 'react';" : "import { Suspense } from 'react';";
  const basenameOption = basePath && basePath !== '/'
    ? `, { basename: '${basePath}' }`
    : '';

  return `// 🐬 Auto-generated by DolphinCSS dolphin-routing
...
export default function dolphincssPlugin() {
  ...
  // Routing state
  let cachedRoutes = [];
  let routingConfig = { enabled: true, pagesDir: 'src/pages', outputFile: 'src/generated-router.jsx', lazyLoad: true, basePath: '/', notFoundComponent: '' };

  async function regenerateRoutes(projectRoot) {
    try {
      // Read routing config from dolphin.config.json
      const configPath = path.join(projectRoot, 'dolphin.config.json');
      if (fs.existsSync(configPath)) {
        try {
          const userConfig = JSON.parse(fs.readFileSync(configPath, 'utf8'));
          if (userConfig.routing) {
            routingConfig = { ...routingConfig, ...userConfig.routing };
          }
        } catch (e) {}
      }

      if (!routingConfig.enabled) {
        console.log('🐬 DolphinCSS: dolphin-routing disabled in config, skipping.');
        return;
      }

      const pagesDir = path.resolve(projectRoot, routingConfig.pagesDir);
      if (!fs.existsSync(pagesDir)) {
        console.log('🐬 DolphinCSS: No pages/ folder found, skipping auto-routing.');
        return;
      }

      const newRoutes = generateRoutesFromFiles(pagesDir, '', projectRoot);

      // Only write if routes actually changed
      if (JSON.stringify(cachedRoutes) === JSON.stringify(newRoutes)) return;
      cachedRoutes = newRoutes;

      const routerCode = generateRouterCode(newRoutes, routingConfig);
      const outputFile = path.resolve(projectRoot, routingConfig.outputFile);

      // Ensure output directory exists
      const outputDir = path.dirname(outputFile);
      if (!fs.existsSync(outputDir)) fs.mkdirSync(outputDir, { recursive: true });

      fs.writeFileSync(outputFile, routerCode, 'utf-8');
      console.log(`\n🔄 DolphinCSS dolphin-routing: ${newRoutes.length} route(s) generated → ${path.relative(projectRoot, outputFile)}`);
      newRoutes.forEach(r => console.log(`   ${r.path}  ←  ${r.file}`));
    } catch (error) {
      console.error('❌ DolphinCSS dolphin-routing error:', error.message);
    }
  }
  ...
  return {
    ...
    configureServer(server) {
      // 🐬 dolphin-routing: watch src/pages/ for changes
      const projectRoot = process.cwd();

      // Read pagesDir from config (default: src/pages)
      let watchDir = path.resolve(projectRoot, 'src/pages');
      try {
        const cfgPath = path.join(projectRoot, 'dolphin.config.json');
        if (fs.existsSync(cfgPath)) {
          const cfg = JSON.parse(fs.readFileSync(cfgPath, 'utf8'));
          if (cfg.routing && cfg.routing.pagesDir) {
            watchDir = path.resolve(projectRoot, cfg.routing.pagesDir);
          }
          if (cfg.routing && cfg.routing.enabled === false) return;
        }
      } catch (e) {}

      // Only watch if pages dir exists (or may be created later)
      const startWatcher = () => {
        if (!fs.existsSync(watchDir)) {
          // Retry after 3s in case user creates folder later
          setTimeout(startWatcher, 3000);
          return;
        }

        // Use Vite's built-in watcher (chokidar) via server.watcher
        server.watcher.add(watchDir);

        const onPagesChange = async (filePath) => {
          if (!filePath.startsWith(watchDir)) return;
          console.log(`\n📁 DolphinCSS dolphin-routing: change detected in pages/ — regenerating...`);
          await regenerateRoutes(projectRoot);
          // HMR full reload so the new router is picked up
          server.ws.send({ type: 'full-reload', path: '*' });
        };

        server.watcher.on('add', onPagesChange);
        server.watcher.on('unlink', onPagesChange);
        server.watcher.on('addDir', onPagesChange);
        server.watcher.on('unlinkDir', onPagesChange);

        console.log(`\n🐬 DolphinCSS dolphin-routing: watching ${path.relative(projectRoot, watchDir)}/ for changes...`);
      };
